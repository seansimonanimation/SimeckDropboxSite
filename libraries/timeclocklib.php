...
echo "<table width='100%' border='0' style='border-collapse: collapse;table-layout:fixed;'><tr><td>User</td><td>Time In</td><td>Time Out</td><td>Shift Length</td></tr>";
echo "</table></div>";
foreach($entryArray as $entry){
    ...
    echo "<td>" . DetermineShiftLengthOrSummonButton($entry['time_in'],$entry['time_out']) . "</td>";
    echo "</tr>";
    echo "</table></div>";
}
...
function DetermineShiftLengthOrSummonButton($timeIn, $timeOut){
    if($timeOut == ''){
        return '<button>Currently Working.<br /> Click to Clock Out</button>';
    }
